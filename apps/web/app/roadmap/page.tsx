...
function RoadmapContent() {
  ...
  if (!metadata.career && !roadmapData?.career) {
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-gradient-to-br from-cyan-50 to-purple-100">
        <div className="text-center max-w-md bg-white p-12 rounded-3xl shadow-2xl">
          <div className="text-6xl mb-4">😕</div>
          <h2 className="text-2xl font-bold mb-4">No roadmap data found</h2>
          <p className="text-gray-600 mb-6">Please complete the quiz to generate your roadmap</p>
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-cyan-50 via-blue-50 to-purple-100">
      {/* Header */}
      <div className="border-b bg-gradient-to-r from-cyan-600 via-blue-600 to-purple-600 text-white shadow-xl">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left Sidebar */}
        <div className="w-full md:w-96 border-r bg-white/80 backdrop-blur-sm overflow-y-auto">
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-5 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 text-sm">Median Salary</span>
                  <span className="font-bold text-xl text-green-700">
                    ${salaryOutlook.median_salary?.toLocaleString() || 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600 text-sm">Job Growth</span>
                  <span className="font-semibold text-green-600">{salaryOutlook.growth_rate || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600 text-sm">Outlook</span>
                  <span className="font-semibold text-gray-900">{salaryOutlook.job_outlook || 'N/A'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border-2 border-gray-200 rounded-2xl p-5 shadow-lg">
              ...
              <div className="space-y-4">
                {(currentPath.steps || []).map((step: any, index: number) => (
                  <div
                    key={index}
                    className="relative pl-6 pb-4 border-l-4 border-cyan-300 last:border-transparent last:pb-0"
                  >
                    <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-cyan-500 border-2 border-white shadow"></div>
                    <div className="font-semibold text-gray-900">{step.description}</div>
                    <div className="text-sm text-gray-600 mt-1">{step.institution}</div>
...
function PathCard({
  ...
  const gradients = {
    cheapest: 'from-green-50 to-emerald-50 border-green-400',
    fastest: 'from-blue-50 to-cyan-50 border-blue-400',
    prestige: 'from-purple-50 to-pink-50 border-purple-400'
  }
    ...
    <button
      ...
      className={`w-full text-left p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 ${
        selected
          ...
          : 'bg-white border-gray-200 hover:border-cyan-300 shadow-sm hover:shadow-md'
      }`}
    >
      ...
      <div className="space-y-2 text-sm">
        <div className="flex items-center justify-between">
          ...
          <span className="font-bold text-gray-900">${cost?.toLocaleString() || 0}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-gray-600 flex items-center gap-1">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-semibold text-gray-900">{roi?.toFixed(1) || 0} years</span>
        </div>
      </div>
    </button>
  )
